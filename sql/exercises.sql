...
SELECT DISTINCT postal_code FROM actor
...
SELECT title, description, rating,   * FROM film WHERE length > 180
...
SELECT payment_id, amount , payment_date FROM payment WHERE >= 05/27/2005;

# 3c. Select the primary key, amount, and payment date columns from the payment table for payments made on 05/27/2005.
SELECT primary_key, amount, payment_date FROM payment_date ON 05/27/2005
...
SELECT * FROM staff WHERE password
...
SELECT payment_id, amount, payment_date FROM payment WHERE DATE_FORMAT(payment_date,'%Y-%m-%d') IN ('2005-05-25', '2005-05-27', '2005-05-29');
